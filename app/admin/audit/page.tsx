...
export default async function AuditLogPage({ searchParams }: { searchParams: Record<string,string|undefined> }) {
  ...
  const where = {} as any; // could extend filters
    ...
    <div className="space-y-8">
      <header className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div className="space-y-1">
          ...
          <p className="text-sm text-neutral-600">Últimas acciones registradas.</p>
        </div>
        <Link href="/admin" className="px-4 py-2 text-sm font-medium transition rounded-md border border-neutral-300 bg-white hover:bg-neutral-50">Panel</Link>
      </header>
            ...
            <tbody className="divide-y">
              {rows.map(r => (
                <tr key={r.id} className="hover:bg-neutral-50">
                  ...
                  <td className="px-3 py-2 text-xs font-mono">{r.entityId || '-'}</td>
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
          {nextCursor && (
            <Link href={`/admin/audit?cursor=${nextCursor}`} className="px-3 py-1 text-xs font-medium rounded-md border border-neutral-300 bg-white hover:bg-neutral-50">Más</Link>
          )}
        </div>
      </div>
    </div>
