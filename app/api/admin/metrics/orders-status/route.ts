...
import { getOrderStatusBreakdown, normalizeRange } from '../../../../../src/lib/metrics';
...
export async function GET(req: Request) {
  try {
    ...
    return NextResponse.json({ range: { from: range.from.toISOString().slice(0,10), to: range.to.toISOString().slice(0,10), granularity: range.granularity }, breakdown });
  } catch (e: any) {
