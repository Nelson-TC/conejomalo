...
import { getUserSeries, normalizeRange } from '../../../../../src/lib/metrics';
...
export async function GET(req: Request) {
  try {
    ...
    const series = await getUserSeries(range);
    return NextResponse.json({ range: { from: range.from.toISOString().slice(0,10), to: range.to.toISOString().slice(0,10), granularity: range.granularity }, series });
  } catch (e: any) {
