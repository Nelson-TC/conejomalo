...
import { getRecentOrders, normalizeRange } from '../../../../../src/lib/metrics';
...
export async function GET(req: Request) {
  try {
    ...
    const items = await getRecentOrders(range, limit > 0 && limit <= 50 ? limit : 8);
    return NextResponse.json({ range: { from: range.from.toISOString().slice(0,10), to: range.to.toISOString().slice(0,10) }, items });
  } catch (e: any) {
