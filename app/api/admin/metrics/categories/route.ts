...
import { getCategoryDistribution, normalizeRange } from '../../../../../src/lib/metrics';
...
export async function GET(req: Request) {
  try {
    ...
    const categories = await getCategoryDistribution(range);
    return NextResponse.json({ range: { from: range.from.toISOString().slice(0,10), to: range.to.toISOString().slice(0,10) }, categories });
  } catch (e: any) {
